...
ll getans(ll num){
    ...
    for(ll i=tmp.size()-1; i>=0; i--) cnt=cnt*10+tmp[i];
    return cnt;
}
/* ll getans(ll n){
    ll tmp=0;
...
void solution(){
    ...
    while(t--){
        ll n;
        cin>>n;
        string ans = to_string(getans(n));
        replace(ans.begin(),ans.end(),'8','9');
        replace(ans.begin(),ans.end(),'7','8');
        replace(ans.begin(),ans.end(),'6','7');
        replace(ans.begin(),ans.end(),'5','6');
        replace(ans.begin(),ans.end(),'4','5');
        cout<<ans<<endl;
    }
}
