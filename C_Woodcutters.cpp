...
void solution()
{
    ...
    nums[0].first = nums[0].second - nums[0].first;
    forloop(i, 2, n)
    {
        if ((nums[i - 1].first - nums[i - 1].second) > max(nums[i - 2].first, nums[i - 2].second))
        {
            ans++;
            int tmp = nums[i - 1].first;
            nums[i - 1].first -= nums[i - 1].second;
            nums[i - 1].second = tmp;
        }
        else if ((nums[i - 1].first + nums[i - 1].second) < nums[i].first)
        {
            ans++;
            nums[i - 1].second += nums[i - 1].first;
        }
        else{
            nums[i-1].second = nums[i-1].first;
        }
