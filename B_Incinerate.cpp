...
void solution(){
    ...
    while(t--){
        int n,k; cin>>n>>k;
        vi health(n),power(n);
        forloop(i,0,n) cin>>health[i];
        forloop(i,0,n) cin>>power[i];
        vpi nums(n);
        forloop(i,0,n){
            nums[i] = make_pair(health[i],power[i]);
        }
        sort(nums.begin(),nums.end());
        int i=0;
        ll powr=k;
        while(k>=0 && i<n){
            while(i<n && nums[i].first<=powr) i++;
            if(i<n && nums[i].second<=k) k-=nums[i].second;
            else if(i<n) break;
            powr+=k;
        }
        if(i<n) NO();
